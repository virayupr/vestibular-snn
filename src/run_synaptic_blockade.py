...
# Block afferent→cerebellar transmission
syn_aff_cer = Synapses(afferent, cerebellar, on_pre='v_post += 0*mV'); syn_aff_cer.connect(j='i % n_cer')

M_in = SpikeMonitor(input_layer); M_aff = SpikeMonitor(afferent); M_cer = SpikeMonitor(cerebellar)
run(duration)
print("Synaptic Blockade → Input:", M_in.num_spikes, "Afferent:", M_aff.num_spikes, "Cerebellar:", M_cer.num_spikes)
